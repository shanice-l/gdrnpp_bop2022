...
class ICBIN_TRAIN_Dataset(object):
    """icbin bop test."""
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/icbin/test
        ...
        self.scenes = [f"{i:06d}" for i in range(1, 3)]
...
SPLITS_ICBIN = dict(
    icbin_train_real=dict(
        name="icbin_train",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/icbin/train"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/icbin/models_cad"),
        objs=ref.icbin.objects,
...
for obj in ref.icbin.objects:
    name = "icbin_{}_train_real".format(obj)
    select_objs = [obj]
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_ICBIN:
        ...
        used_cfg = data_cfg
    DatasetCatalog.register(name, ICBIN_TRAIN_Dataset(used_cfg))
