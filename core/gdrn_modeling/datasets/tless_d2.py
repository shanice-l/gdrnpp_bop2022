...
class TLESS_TRAIN_Dataset(object):
    """tless bop test."""
    ...
    for split in ["train_primesense", "train_primesense_rescaled"]:
        ...
        if name not in SPLITS_TLESS:
            SPLITS_TLESS[name] = dict(
                name=name,
                dataset_root=osp.join(DATASETS_ROOT, f"BOP_DATASETS/tless/{split}"),
                models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/tless/models_cad"),
                objs=[obj],
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
                with_depth=with_depth,  # (load depth path here, but may not use it)
                height=height,
                width=width,
                cache_dir=osp.join(PROJ_ROOT, ".cache"),
                use_cache=True,
