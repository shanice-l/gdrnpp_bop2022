...
class BasicEncoder(nn.Module):
    def __init__(self, input_dim=3, output_dim=128, norm_fn='batch', dropout=0.0, multidim=False, dres=8):
        ...
        self.conv1 = nn.Conv2d(input_dim, DIM, kernel_size=7, stride=(1 if dres<8 else 2), padding=3)
        self.relu1 = nn.ReLU(inplace=True)

        self.in_planes = DIM
        self.layer1 = self._make_layer(DIM,  stride=1)
        self.layer2 = self._make_layer(2*DIM, stride=(1 if dres<4 else 2))
        self.layer3 = self._make_layer(4*DIM, stride=(1 if dres<2 else 2))
