...
def test_negative_stride_from_python(msg):
    ...
    When a function takes an Eigen matrix by copy or const reference, we
    can pass a numpy array that has negative strides.  Otherwise, an
    exception will be thrown as Eigen will not be able to map the numpy
    array.
    """
...
def test_eigen_return_references():
    """Tests various ways of returning references and non-referencing
    copies."""
...
def test_eigen_ref_life_support():
    ...
    The `Ref` caster sometimes creates a copy which needs to stay alive.
    This needs to happen both for directs casts (just the array) or
    indirectly (e.g. list of arrays).
    """
...
def test_issue738():
    """Ignore strides on a length-1 dimension (even if they would be
...
def test_issue1105():
    """Issue 1105: 1xN or Nx1 input arrays weren't accepted for eigen compile-
    time row vectors or column vector."""
...
def test_custom_operator_new():
    """Using Eigen types as member variables requires a class-specific operator
    new with proper alignment."""
