...
"""
    ...
    def merge_all_tex_files(self):
        tex_files = glob.glob(osp.join(self.eval_dir, "latex", "*.tex"))
    ...
    def include_all_figures(self):
        pdf_files = glob.glob(osp.join(self.eval_dir, "figures", "*.pdf"))
        png_files_eval = glob.glob(osp.join(self.eval_dir, "figures", "*.png"))
        png_files = glob.glob(osp.join(self.log_dir, "train_figures", "*29999.png"))
...
"""
    ...
    def save(self, pdf=True, filename="report.tex", open_pdf=True):
        ...
        full_filename = osp.join(self.eval_dir, "latex", "report.tex")
        ...
        if pdf:
            ...
            # check_output(['pdflatex', filename], cwd=osp.dirname(full_filename))
            cmd = f"cd {osp.dirname(full_filename)}; pdflatex {filename}"
            os.system(cmd)
            if open_pdf:
                Popen(
                    ["evince", filename.split(".")[0] + ".pdf"],
                    cwd=osp.dirname(full_filename),
                )
