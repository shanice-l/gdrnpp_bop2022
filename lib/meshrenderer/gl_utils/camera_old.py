...
def _calc_calib_proj(K, x0, y0, w, h, nc, fc, window_coords="y_down", offset_u0=0, offset_v0=0):
    ...
    # will generate window coords that compensate for the flipped image coords.
        ...
        proj = np.array(
            [
                [2 * fx / w, -2 * s / w, (-2 * u0 + w + 2 * x0) / w, 0],
                [0, 2 * fy / h, (2 * v0 - h + 2 * y0) / h, 0],
                [0, 0, q, qn],  # Sets near and far planes (glPerspective).
                [0, 0, -1, 0],
            ]
        )
...
class Camera(object):
    ...
    #     x0, y0 = (0, 0) if originIsInTopLeft else (-W / 2.0, -H / 2.0)
    #     # match the method of renderer in bop_toolkit
    #     calib_proj = _calc_calib_proj(I, x0, y0, W, H, near, far, window_coords='y_down',
    #         offset_u0=self.offset_u, offset_v0=self.offset_v).T
    #     calib_proj[1,:] *= -1
    #     self.__T_proj_view[:] = calib_proj
