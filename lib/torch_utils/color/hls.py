...
def rgb_to_hls(image: torch.Tensor) -> torch.Tensor:
    ...
    s: torch.Tensor = torch.where(
        l < 0.5,
        deltac / (maxc + minc),
        deltac / (torch.tensor(2.0) - (maxc + minc)),
    )  # saturation
    ...
    image_hls = torch.where(
        torch.isnan(image_hls),
        torch.tensor(0.0, device=image_hls.device, dtype=image_hls.dtype),
        image_hls,
    )
...
def hls_to_rgb(image: torch.Tensor) -> torch.Tensor:
    ...
    fr: torch.Tensor = l - a * torch.max(
        torch.min(torch.min(kr - torch.tensor(3.0), torch.tensor(9.0) - kr), ones_k),
        -1 * ones_k,
    )
    fg: torch.Tensor = l - a * torch.max(
        torch.min(torch.min(kg - torch.tensor(3.0), torch.tensor(9.0) - kg), ones_k),
        -1 * ones_k,
    )
    fb: torch.Tensor = l - a * torch.max(
        torch.min(torch.min(kb - torch.tensor(3.0), torch.tensor(9.0) - kb), ones_k),
        -1 * ones_k,
    )
